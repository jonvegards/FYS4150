...
void SaveResultsFromSeveralMCCycles(string FileName, double *HeatCapacity, double *Susceptibility, double *MeanMagnetization, double *MC_cycles, int N, double *, double *, double *);
...
void RunForDifferentMCCycles();
void RunForDifferentTemperatures(int argc, char *argv[]);


int main(int argc, char* argv[]){
	//RunForDifferentMCCycles();
	RunForDifferentTemperatures(argc, argv);
    return 0;
}
void metropolis_algo(int **state_matrix, int L_spins, double &energy, double &magnetization, double *w, long &idum, int &accepted, int **neighbour){
...
void RunForDifferentMCCycles() {
	...
	string FileName = "RunningWithSeveralMCOrderedStartT24_blabla.m";
	//------------------------------------------------------
	...
	CreateStartingState(state_matrix, L, energy, magnetization, idum, random);
	...
	for(int q=0;q<N;q++){
		...
		ExpectationValues(values, MC_cycles_no[q], accepted_total[q], HeatCapacity[q], Susceptibility[q], MeanMagnetization[q], L);
		// Calculate probabilities for having a certain state after reaching equilibrium
		if( MC_cycles_no[q] > 40000 ) ProbabilityForGivenEnergy(state_matrix, L, probability[q], neighbour);
		finish = clock();
	...
	SaveResultsFromSeveralMCCycles(FileName, HeatCapacity,Susceptibility, MeanMagnetization, MC_cycles_no, N, time_used, accepted_total, probability);
}
